...
function AdminUpdate({
  ...
  const updateArticleStatus = async (e) => {
    ...
    const response = await fetch(`/api/v1/articles/admin/`, options).catch(
      handleError
    );
  ...
  const list = (
    <button className="articleButton" onClick={() => setStatus(true)}>
      Change status
    </button>
  );
  ...
  const statusView = (
    <div>
      <Button value="REJECTED" onClick={updateArticleStatus}>
        Reject
      </Button>
      <Button value="PUBLISHED" onClick={updateArticleStatus}>
        Publish
      </Button>
      <Button value="ARCHIVED" onClick={updateArticleStatus}>
        Archive
      </Button>
  ...
  const changeCategory = (
    <Button className="articleButton" onClick={() => setSection(true)}>
  ...
  const selectCategory = (
    <div>
      ...
      <Button className="articleButton" onClick={() => setSection(false)}>
        Back
      </Button>
    </div>
  );
